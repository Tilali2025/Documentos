...
---------------------------------------------------------------------------------------
	...
	@MOTORIZADO					VARCHAR(50)		= '' ,
	@FARMACIA					VARCHAR(50)		= '' ,
	@CLASIFICACION				VARCHAR(50)		= '' ,
	@ESTADO						CHAR(1)			= '' ,
	@CRITERIO_BUSQUEDA			VARCHAR(50)		= '' 
	AS
		BEGIN
			SET NOCOUNT ON
			...
			SELECT 
				...
				IF(@FECHA_INICIO <> '' AND @FECHA_FINAL <> '')   													SET @vFILTRO += ' AND PED.DPED_FEC_PED BETWEEN ''' + CONVERT(VARCHAR,@FECHA_INICIO,23) +  ''' AND ''' + CONVERT(VARCHAR,@FECHA_FINAL,23) + ''' ' + CHAR(13)
				-- FILTRO SEGUN EL CAMPO SELECCIONADO
				IF(@CAMPO = 'PEDIDO' AND @CRITERIO_BUSQUEDA <> '' OR @CRITERIO_BUSQUEDA = NULL)  					SET @vFILTRO += ' AND PED.CPED_ID = ''' + @CRITERIO_BUSQUEDA + ''' ' + CHAR(13)
				IF(@CAMPO = 'ATENCION' AND @CRITERIO_BUSQUEDA <> '' OR @CRITERIO_BUSQUEDA = NULL)  					SET @vFILTRO += ' AND PED.CPED_CODIGO_HISTORIACLINICA = ''' + @CRITERIO_BUSQUEDA + ''' ' + CHAR(13)
				IF(@CAMPO = 'PEDIDO N' AND @CRITERIO_BUSQUEDA = NULL)							  					SET @vFILTRO += ' AND PED.SPED_FLAG_TP = ' + 'N' + CHAR(13)
				IF(@CAMPO = 'PEDIDO D' AND @CRITERIO_BUSQUEDA = NULL)							  					SET @vFILTRO += ' AND PED.SPED_FLAG_TP = ' + 'D' + CHAR(13)
				IF(@CAMPO = 'PEDIDO AUDITADO' AND @CRITERIO_BUSQUEDA = NULL)							  			SET @vFILTRO += ' AND CONVERT(VARCHAR,PED.NPED_CONDICION_PEDIDO) = ' + '0' + CHAR(13)
				IF(@CAMPO = 'PACIENTE' AND @CRITERIO_BUSQUEDA <> '' OR @CRITERIO_BUSQUEDA = NULL)  					SET @vFILTRO += ' AND SPER_NOMBRES + '' '' + SPER_APELLIDO_MATERNO + '' '' + SPER_APELLIDO_MATERNO LIKE  ''' + CAST(@CRITERIO_BUSQUEDA AS VARCHAR) + '%'' ' + CHAR(13)
				IF(@CAMPO = 'DISTRITO' AND @CRITERIO_BUSQUEDA <> '' OR @CRITERIO_BUSQUEDA = NULL)  					SET @vFILTRO += ' AND U.SUBI_DISTRITO = ' + CAST(@CRITERIO_BUSQUEDA AS VARCHAR) + CHAR(13)
				IF(@CAMPO = 'DNI' AND @CRITERIO_BUSQUEDA <> '' OR @CRITERIO_BUSQUEDA = NULL)  						SET @vFILTRO += ' AND PP.SPER_NUMERO_DOCUMENTO = ' + CAST(@CRITERIO_BUSQUEDA AS VARCHAR) + CHAR(13)
				--FILTRO POR OTRAS CONDICIONES
				IF(@MOTORIZADO <> '')  																				SET @vFILTRO += ' AND PMO.CMOT_ID = ''' + @MOTORIZADO + ''' ' + CHAR(13)
				IF(@FARMACIA <> '' )  																				SET @vFILTRO += ' AND F.CFAR_CODFAR = ''' + @FARMACIA + ''' ' +  CHAR(13)
				IF(@CLASIFICACION <> '')														  					SET @vFILTRO += ' AND SN.CSER_ID = ''' + RTRIM(@CLASIFICACION) + ''' ' +  CHAR(13)
				IF(@ESTADO <> ''	)  																				SET @vFILTRO += ' AND PED.NPED_ESTADO = ''' + @ESTADO + ''' ' + CHAR(13)
